...
import { TFormFields } from "./chatSection";
import { Camera, ImagePlus, Send } from "lucide-react";


type TInputSection = {
    handleSubmit: (onSubmit: SubmitHandler<TFormFields>) => (event: React.FormEvent<HTMLFormElement>) => void;
    onSubmit: SubmitHandler<TFormFields>;  // Adjusted to return void
    register: UseFormReturn<TFormFields>['register'];
    handleKeyDown: React.KeyboardEventHandler<HTMLTextAreaElement>;
  };

export default function InputSection({handleSubmit,handleKeyDown,onSubmit,register}:TInputSection) {
    return <>
